...
# def generate_from_json_schema(jsonSchema, path):
#     log(f"Generating from JSON schema at {'.'.join(path)}")
#     return generate_value(jsonSchema, path)

# def generate_openapi_object(openapiSchema, path):
#     props = openapiSchema.get("properties", {})
#     obj = {}

#     for k, v in props.items():
#         new_path = path + [k]

#         # Handle schema injections at precise OpenAPI locations
#         if new_path == ["resource", "reporterData", "resourceData"]:
#             obj[k] = generate_from_json_schema(reporter_schema, new_path)
#             continue

#         if new_path == ["resource", "commonResourceData"]:
